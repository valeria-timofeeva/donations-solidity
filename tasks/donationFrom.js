require("@nomiclabs/hardhat-ethers");

const contractAddress = '0x2E65Ff62B56E30581CdaC82c1c4cd3C75bAbeADe';
const Donations = require('./Donation.json');

task('donationsFrom', 'Amount of collected donations')
    .addParam('address', 'Address of donator')
    .setAction(async (taskArgs) => {
        
        console.log('Calculating...');
        const [owner] = await ethers.getSigners();
        const donations = new ethers.Contract(
            contractAddress,
            Donations.abi,
            owner
        );

        const result = await donations.donations(taskArgs.address);
        console.log('Amount of collected donations:', result);
    });